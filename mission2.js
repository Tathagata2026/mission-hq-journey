// Buttons
const teamButton = document.getElementById("teamBtn");
const lessonButton = document.getElementById("lessonBtn");
const project1Button = document.getElementById("project1Btn");
const project2Button = document.getElementById("project2Btn");

// Output area
const output = document.getElementById("output");

// Event listeners
// Meet the team
teamButton.addEventListener("click", function (event) {

showContent(`
    <h3>Meet the Team</h3>
    <ul>
      <li>Ewan Zhang - Trainer</li>
      <li>Jordan Meijer - Assistant Trainer</li>
      <li>Adam Tung - Assistant Trainer</li>
      <li>Jason Cluley - Capability Development Lead</li>
    </ul>
  `);
});

// Lessons Learned
lessonButton.addEventListener("click", function (event) {
  showContent(`
    <h3>Lessons Learned</h3>
    <p>I learned HTML for structure, CSS for styling, and JavaScript to make websites interactive.</p>
  `);
});

// Project 1
project1Button.addEventListener("click", function (event) {
 showContent(`
    <h3>Project 1</h3>
    <p>A webpage built using HTML and CSS.</p>
  `);
});

//project 2
project2Button.addEventListener("click", function (event) {
  showContent(`
    <h3>Project 2</h3>
    <p>A webpage built using HTML, CSS, and JavaScript with interactive features.</p>
  `);
});
// Show content function
function showContent(html) {
  hideOutput();
  output.innerHTML = html;
  output.style.display = "";
}

// Function to hide output
function hideOutput() {
  output.style.display = "none";
  output.innerHTML = "";
}




const tools = [
{ name: "VS Code", logo: "https://images-eds-ssl.xboxlive.com/image?url=4rt9.lXDC4H_93laV1_eHHFT949fUipzkiFOBH3fAiZZUCdYojwUyX2aTonS1aIwMrx6NUIsHfUHSLzjGJFxxj7kCzMIlSC20SNjaJf9GmG15ocnF.zbBRgxMSlB7Ejh6FbgNzxLvZOoW7N3ML56fn3m5Z4MO.M8pYrCFVKIhqM-&format=source" },
{ name: "GitHub", logo: "data:image/png;base64," },
{ name: "JavaScript", logo: "https://www.w3schools.com/js/img_javascript_480.jpg" }
];
// elements
const logo = document.getElementById("toolLogo");
const name = document.getElementById("toolName");

let index = 0;

// loop that runs every 2 seconds
setInterval(function(){

logo.src = tools[index].logo;
// move to next logo
index++;
if(index >= tools.length){
index = 0;
}
}, 4000);